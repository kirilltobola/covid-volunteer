...
class UserRepository : UserApi {
    override suspend fun getUser(token: String, id: Long): UserDto {
        return Retrofit.userApi.getUser(token, id)
    }

    override suspend fun getUserOrders(token: String, id: Long): List<OrderDto> {
        return Retrofit.userApi.getUserOrders(token, id)
    }

    override suspend fun editUser(token: String, id: Int, user: UserDto) {
        return Retrofit.userApi.editUser(token, id, user)
    }

    override suspend fun changePassword(token: String, id: Int, password: ChangePasswordDto) {
